...
payload_loop_init ()
{
    ...
    struct payload_config *pl_cfg = payload_config_ref();

    assert (cfg->interface != NULL);

    /* get network number and mask associated with capture device */
    if (pcap_lookupnet(cfg->interface, &pl_cfg->net, &pl_cfg->mask, pl_cfg->errbuf) != 0)
    {
        D_WARNING("pcap_lookupnet(): Couldn't get ip/netmask, %s\n", cfg->interface, pl_cfg->errbuf);
        return -1;
    }
...
payload_loop_run()
{
    ...
    /* open capture device */
    pl_cfg->handle = pcap_open_live (cfg->interface, SNAP_LEN, 1, 1000, pl_cfg->errbuf);
                           ...
                           const u_char *packet)
{
    ...
    //     D_DEBUG4("WARNING: The packet (%04x) is unsupported! ETHERTYPE_IP=%04x ETHERTYPE_VLAN=%04x\n",
    //      ether_type, ETHERTYPE_IP, ETHERTYPE_VLAN);
    //     return;
    // }
    ...
    D_DEBUG1("[GRE:%s] the client %s was attached in %s\n", 
                cfg->interface, inet_ntoa(tun->ip_remote), tun->ifgre);
